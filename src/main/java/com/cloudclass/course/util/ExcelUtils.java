...
import com.cloudclass.course.exception.ExcelException;
import jxl.Cell;
import jxl.Sheet;
import jxl.Workbook;
...
public class ExcelUtils {
    ...
    public static <T>  void   listToExcel1 (
        ...
        try {
            ...
            for(int i=0; i<sheetNum; i++){
                ...
                if(1==sheetNum){
                    WritableSheet sheet=wwb.createSheet(sheetName, i);
                    fillSheet1(sheet, list, fieldMap, 0, list.size()-1);
                    ...
                    int lastIndex=(i+1)*sheetSize-1>list.size()-1 ? list.size()-1 : (i+1)*sheetSize-1;
                    //填充工作表
                    fillSheet1(sheet, list, fieldMap, firstIndex, lastIndex);
                }
            }
    ...
    private static <T> void fillSheet1(
            WritableSheet sheet,
            List<T> list,
            LinkedHashMap<String,String> fieldMap,
            int firstIndex,
        ...
        for(int i=0;i<cnFields.length;i++){
            WritableFont font2 = new WritableFont(WritableFont.ARIAL,10,WritableFont.BOLD,false, UnderlineStyle.NO_UNDERLINE,Colour.BLACK);
            WritableCellFormat cellFormat2 = new WritableCellFormat(font2);
            cellFormat2.setAlignment(Alignment.CENTRE);
            cellFormat2.setVerticalAlignment(VerticalAlignment.CENTRE);
            Label label=new Label(i,0,cnFields[i],cellFormat2);
            sheet.addCell(label);
        }
        ...
        for(int index=firstIndex;index<=lastIndex;index++){
            if(index+1 <= lastIndex){
                ...
                T item1=list.get(index+1);
                Object objValue=getFieldValueByNameSequence(enFields[0], item);
                Object objValue1=getFieldValueByNameSequence(enFields[0], item1);
                if(objValue.equals(objValue1)){
                    a += 1;
                }else{
                    la.add(a);
                    a=1;
        ...
        for (int index=0;index<la.size();index++) {
            ...
            sheet.mergeCells(0,c,0,d);
            sheet.mergeCells(1,c,1,d);
            sheet.mergeCells(2,c,2,d);
            sheet.mergeCells(3,c,3,d);
            sheet.mergeCells(4,c,4,d);
            sheet.mergeCells(7,c,7,d);
            sheet.mergeCells(10,c,10,d);
            sheet.mergeCells(11,c,11,d);
        }
        //填充内容
        int rowNo=1;
        for(int index=firstIndex;index<=lastIndex;index++){
    ...
    public static  <T>  void   listToExcel1 (
        ...
        listToExcel1(list, fieldMap, sheetName, 65535, out);
     ...
     * @throws ExcelException
     */
    public static <T>  List<T>  excelToList1(
            InputStream in,
            String sheetName,
            Class<T> entityClass,
            LinkedHashMap<String, String> fieldMap,
